...
## here is the example of outliner
outlier = num_friends.index(100) #this is very abnormal in figure5.1
num_friends_good = [ x
                    for x in enumerate(num_firends)
                    if i != outlier]
dialy_minutes_good = [ x
                    for x in emumerate(num_firends)
                    if i != outlier]
correlation(num_friends_good, dialy_minutes_good) # so we just leave out the outliers
